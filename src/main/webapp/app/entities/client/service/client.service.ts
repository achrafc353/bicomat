...
export class ClientService {
  public resourceUrl = this.applicationConfigService.getEndpointFor('api/clients');
  ...
  protected convertDateFromClient(client: IClient): IClient {
    return Object.assign({}, client, {
      anneeArrivee: client.anneeArrivee?.isValid() ? client.anneeArrivee.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.anneeArrivee = res.body.anneeArrivee ? dayjs(res.body.anneeArrivee) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((client: IClient) => {
        client.anneeArrivee = client.anneeArrivee ? dayjs(client.anneeArrivee) : undefined;
      });
    }
    return res;
  }
