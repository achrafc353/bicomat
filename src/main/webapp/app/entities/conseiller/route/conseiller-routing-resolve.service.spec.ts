jest.mock('@angular/router');

import { TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRouteSnapshot, Router } from '@angular/router';
import { of } from 'rxjs';

import { IConseiller, Conseiller } from '../conseiller.model';
import { ConseillerService } from '../service/conseiller.service';

import { ConseillerRoutingResolveService } from './conseiller-routing-resolve.service';

describe('Service Tests', () => {
  describe('Conseiller routing resolve service', () => {
    let mockRouter: Router;
    let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
    let routingResolveService: ConseillerRoutingResolveService;
    let service: ConseillerService;
    let resultConseiller: IConseiller | undefined;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        providers: [Router, ActivatedRouteSnapshot],
      });
      mockRouter = TestBed.inject(Router);
      mockActivatedRouteSnapshot = TestBed.inject(ActivatedRouteSnapshot);
      routingResolveService = TestBed.inject(ConseillerRoutingResolveService);
      service = TestBed.inject(ConseillerService);
      resultConseiller = undefined;
    });

    describe('resolve', () => {
      it('should return IConseiller returned by find', () => {
        // GIVEN
        service.find = jest.fn(id => of(new HttpResponse({ body: { id } })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultConseiller = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultConseiller).toEqual({ id: 123 });
      });

      it('should return new IConseiller if id is not provided', () => {
        // GIVEN
        service.find = jest.fn();
        mockActivatedRouteSnapshot.params = {};

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultConseiller = result;
        });

        // THEN
        expect(service.find).not.toBeCalled();
        expect(resultConseiller).toEqual(new Conseiller());
      });

      it('should route to 404 page if data not found in server', () => {
        // GIVEN
        spyOn(service, 'find').and.returnValue(of(new HttpResponse({ body: null })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultConseiller = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultConseiller).toEqual(undefined);
        expect(mockRouter.navigate).toHaveBeenCalledWith(['404']);
      });
    });
  });
});
